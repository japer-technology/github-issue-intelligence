...
The workflow also respects the `.issue-intelligence/ISSUE-INTELLIGENCE-ENABLED.md` sentinel file. See [Disable](disable.md) and [Enable](enable.md) for details.
...
Also update `.issue-intelligence/.pi/settings.json` to match (see [Configure](configure.md)).
...
The installer copies the workflow template from `.issue-intelligence/install/ISSUE-INTELLIGENCE-WORKFLOW-AGENT.yml`.
...
The sentinel file `.issue-intelligence/ISSUE-INTELLIGENCE-ENABLED.md` is missing. See [Enable](enable.md) to restore it.
...
- Check that `.issue-intelligence/package.json` and `.issue-intelligence/bun.lock` are present and valid
- Try running `cd .issue-intelligence && bun install` locally to reproduce the error
